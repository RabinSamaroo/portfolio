        ...
        <aside className="lg:w-1/2 w-full flex flex-col items-center justify-center px-8 py-8 lg:py-16 lg:fixed lg:h-full lg:justify-between border-b lg:border-b-0 lg:border-r border-foreground/10 bg-background z-10 shadow-none lg:shadow-xl">
          <div className="flex flex-col items-center lg:items-start w-full max-w-md mx-auto">
            <h1 className="text-4xl lg:text-5xl font-extrabold tracking-tight mb-1 text-foreground">
              {rabin.firstName} <span className="text-accent">{rabin.lastName}</span>
            </h1>
            ...
            <div className="mb-6 w-full">
              ...
              <div className="flex flex-col gap-1 text-xs text-foreground/70 mb-3">
                <span><span className="font-semibold text-foreground">Email:</span> <a href={`mailto:${rabin.contact.email}`} className="underline hover:text-accent transition-colors">{rabin.contact.email}</a></span>
                <span><span className="font-semibold text-foreground">Phone:</span> {rabin.contact.phone}</span>
                <span><span className="font-semibold text-foreground">Location:</span> {rabin.contact.location}</span>
                <span><span className="font-semibold text-foreground">Website:</span> <a href={rabin.contact.websiteUrl} target="_blank" rel="noopener noreferrer" className="underline hover:text-accent transition-colors">{rabin.contact.websiteUrl.replace(/^https?:\/\//, "")}</a></span>
              </div>
              <div className="flex flex-wrap gap-2 text-xs mb-3">
                {rabin.languages.map((lang: { name: string; level: string }) => (
                  <span key={lang.name} className="bg-foreground/10 rounded-full px-3 py-1 font-medium text-foreground/80 border border-foreground/10">{lang.name} <span className="text-foreground/50">({lang.level})</span></span>
                ))}
              </div>
              <div className="flex flex-wrap gap-1 text-xs">
                {rabin.skills.map((skill: string) => (
                  <span key={skill} className="bg-foreground/5 rounded-full px-3 py-1 font-medium text-foreground/80 border border-foreground/10 mb-1">{skill}</span>
                ))}
              </div>
            </div>
